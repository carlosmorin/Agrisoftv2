...
module Crm::SalesHelper
  ...
  def total_by_freight(_price, _percentage)
    get_porcent
  end
  ...
  def total_by_package(price, _percentage)
    @sale.n_products * price
  end

  def total_by_freight(price, _percentage)
    price
  end

  def currency(price, _code = nil)
  ...
  def type_expense(expense)
    if expense.expense_type?
      return "<span class='badge badge-primary right s-10'> Gasto </span>"
 end

    if expense.discount_type?
      "<span class='badge badge-info right s-10'> Descuento </span>"
 end
  end
