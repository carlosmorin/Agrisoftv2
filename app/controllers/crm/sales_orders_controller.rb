  ...
  class SalesOrdersController < ApplicationController
    ...
    def index
      ...
      if params[:tab] == 'actives'
        @order_sales = Shipment.left_outer_joins(:appointments).active_order_sales
       end
      if params[:tab] == 'expirated'
        @order_sales = Shipment.joins(:appointments).expirated_order_sales
       end
    ...
    def order_sale_params
      params.require(:shipment).permit(:client_id, :company_id, :contact_id,
                                       ...
                                       shipments_products_attributes: %i[id price quantity shipment_id product_id productable_type productable_id _destroy],
                                       appointments_attributes: %i[id n_request started_at finished_at appointment_at commitment_at appointment_number comments])
    end
