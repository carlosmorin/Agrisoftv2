...
class Config::Production::TreatmentsController < ApplicationController
  ...
  def create
    ...
    if @treatment.save
      ...
      if params[:desease_id].present?
        return redirect_to config_production_desease_path(params[:desease_id], tab: :treatments)
      end
      if params[:pest_id].present?
        return redirect_to config_production_pest_path(params[:pest_id], tab: :treatments)
      end
  ...
  def destroy
    if params[:desease_id].present?
      ...
      @desease.treatments.where(id: params[:id]).each(&:destroy)
    end

    if params[:pest_id].present?
      @pest = Pest.find(params[:pest_id])
      @pest.treatments.where(id: params[:id]).each(&:destroy)
    end

    @treatment.destroy if params[:supply_id].present?
  end
  ...
  def treatment_params
    params.require(:treatment).permit(
      ...
      treatment_supplies_attributes: %i[
        id
        supply_id
        foliar_quantity
        foliar_unit
  ...
  def add_custom_breadcrumb
    ...
    if @desease.present?
      add_breadcrumb 'Enfermedad', config_production_desease_url(@desease, tab: :treatments)
    end
    if @pest.present?
      add_breadcrumb 'Plaga', config_production_pest_url(@pest, tab: :treatments)
    end
  end
  ...
  def redirect_to_current_resource
    if params[:desease_id].present?
      return redirect_to new_config_production_desease_treatment_path(desease_id: params[:desease_id])
    end
    if params[:pest_id].present?
      return redirect_to new_config_production_pest_treatment_path(pest_id: params[:pest_id])
    end

    if params[:supply_id].present?
      redirect_to new_config_production_supply_treatment_path(supply_id: params[:supply_id])
    end
  end
