class Config::Production::HostsController < ApplicationController
  ...
  def get_redirect
    return redirect_to config_production_pest_path(@pest, tab: :hosts) if @pest.present?
    return redirect_to config_production_desease_path(@desease, tab: :hosts) if @desease.present?
  ...
  def host_params
    params.require(:host).permit(:name, :description, :picture, :pest_id, :desease_id,
      ...
      deseases_hosts_attributes: [:id, :host_id, :desease_id, :_destroy])
  end
